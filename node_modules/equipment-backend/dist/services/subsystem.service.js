...
export async function getAllSubsystems() {
    const result = await query(`SELECT subsys_id, name, location, note, parent_id, created_at, updated_at
     FROM equipment.subsystems
     WHERE is_active = true
     ORDER BY name`);
    return result.rows;
}
export async function getSubsystemById(subsysId) {
    const result = await query(`SELECT subsys_id, name, location, note, parent_id, created_at, updated_at
     FROM equipment.subsystems
     WHERE subsys_id = $1 AND is_active = true`, [subsysId]);
    return result.rows[0] || null;
}
export async function getSubsystemTree() {
    const result = await query(`WITH RECURSIVE subsystem_tree AS (
...
export async function getSubsystemNodes(subsysId) {
    const result = await query(`SELECT n.node_id, n.name, n.manufacturer, n.model, n.status, n.location,
     ...
     ORDER BY n.name`, [subsysId]);
    return result.rows;
}
export async function createSubsystem(data, userId, ipAddress, userAgent) {
    const result = await query(`INSERT INTO equipment.subsystems (subsys_id, name, location, note, parent_id, is_active)
     VALUES (uuid_generate_v4(), $1, $2, $3, $4, true)
     RETURNING subsys_id, name, location, note, parent_id`, [data.name, data.location, data.note || null, data.parent_id || null]);
...
export async function updateSubsystem(subsysId, data, userId, ipAddress, userAgent) {
    ...
    await query(`UPDATE equipment.subsystems SET ${updates.join(', ')} WHERE subsys_id = $${paramIndex}`, values);
...
export async function deleteSubsystem(subsysId, userId, ipAddress, userAgent) {
    const oldData = await getSubsystemById(subsysId);
    if (!oldData) {
        throw new Error('Подсистема не найдена');
    }
    // Проверяем, есть ли дочерние подсистемы или узлы
    const childrenCheck = await query(`SELECT 
       (SELECT COUNT(*) FROM equipment.subsystems WHERE parent_id = $1 AND is_active = true) as child_subsystems,
       (SELECT COUNT(*) FROM equipment.nodes WHERE subsystem_id = $1 AND write_off_date IS NULL) as child_nodes`, [subsysId]);
    ...
    await query(`UPDATE equipment.subsystems SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE subsys_id = $1`, [subsysId]);
