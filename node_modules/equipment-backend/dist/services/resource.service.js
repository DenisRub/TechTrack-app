...
export async function getResourceById(resourceId) {
    const result = await query(`SELECT r.resource_id, r.node_id, r.registration_date, r.resource_params, r.note,
            n.name as node_name,
            n.manufacturer as node_manufacturer,
            n.model as node_model,
            n.location as node_location,
     ...
     FROM equipment.resources r
     JOIN equipment.nodes n ON r.node_id = n.node_id
     WHERE r.resource_id = $1 AND n.write_off_date IS NULL`, [resourceId]);
    return result.rows[0] || null;
}
export async function getResourcesForNode(nodeId) {
    const result = await query(`SELECT resource_id, registration_date, resource_params, note
     FROM equipment.resources
     WHERE node_id = $1
     ORDER BY registration_date DESC`, [nodeId]);
    return result.rows;
}
export async function createResource(data, userId, ipAddress, userAgent) {
    ...
    const result = await query(`INSERT INTO equipment.resources (resource_id, node_id, registration_date, resource_params, note)
     VALUES (uuid_generate_v4(), $1, $2, $3, $4)
     RETURNING resource_id, node_id, registration_date`, [data.node_id, data.registration_date, data.resource_params || {}, data.note || null]);
...
export async function updateResource(resourceId, data, userId, ipAddress, userAgent) {
    ...
    await query(`UPDATE equipment.resources SET ${updates.join(', ')} WHERE resource_id = $${paramIndex}`, values);
...
export async function deleteResource(resourceId, userId, ipAddress, userAgent) {
    const oldData = await getResourceById(resourceId);
    if (!oldData) {
        throw new Error('Ресурс не найден');
    }
...
export async function calculateResource(nodeId, workHoursPerYear) {
    // Получаем узел и его ресурс
    const nodeResult = await query(`SELECT n.node_id, n.name, n.operation_mode,
            r.resource_params
