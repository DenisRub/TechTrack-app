import { getAllSubsystems, getSubsystemById, getSubsystemTree, getSubsystemNodes, createSubsystem, updateSubsystem, deleteSubsystem, } from '../services/subsystem.service.js';
export async function getSubsystemsController(req, res) {
    try {
...
export async function getSubsystemController(req, res) {
    try {
...
export async function getSubsystemTreeController(req, res) {
    try {
...
export async function getSubsystemNodesController(req, res) {
    try {
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
...
export async function createSubsystemController(req, res) {
    try {
...
export async function updateSubsystemController(req, res) {
    try {
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const userId = req.user?.user_id || null;
        const ipAddress = req.ip || req.socket.remoteAddress || null;
...
export async function deleteSubsystemController(req, res) {
    try {
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const userId = req.user?.user_id || null;
        const ipAddress = req.ip || req.socket.remoteAddress || null;
