...
export async function getEntityAuditHistory(entityType, entityId) {
    const result = await query(`SELECT l.log_id, l.user_id, l.action, l.old_data, l.new_data, l.performed_at,
            u.login, u.full_name
     FROM equipment.audit_log l
     LEFT JOIN equipment.users u ON l.user_id = u.user_id
     WHERE l.entity_type = $1 AND l.entity_id = $2
     ORDER BY l.performed_at DESC`, [entityType, entityId]);
    return result.rows;
}
