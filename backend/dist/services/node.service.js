...
export async function getNodeById(nodeId) {
    const result = await query(`SELECT n.node_id, n.node_type_id, n.name, n.manufacturer, n.model, 
            n.serial_number, n.inventory_number, n.registration_number,
            n.status, n.location, n.parameters, n.note, n.installed_in_node, n.subsystem_id,
            n.commission_date, n.operation_mode, n.decommission_date, n.write_off_date,
            n.manufactured_date,
     ...
     JOIN equipment.subsystems s ON n.subsystem_id = s.subsys_id
     LEFT JOIN equipment.nodes parent ON n.installed_in_node = parent.node_id
     WHERE n.node_id = $1 AND n.write_off_date IS NULL`, [nodeId]);
    if (result.rows.length === 0)
        return null;
    return result.rows[0];
}
export async function getNodeTree() {
    const result = await query(`WITH RECURSIVE node_tree AS (
...
export async function getNodeChildren(nodeId) {
    const result = await query(`SELECT n.node_id, n.name, n.manufacturer, n.model, n.status, n.location,
            nt.name as node_type_name
     FROM equipment.nodes n
     JOIN equipment.node_types nt ON n.node_type_id = nt.node_type_id
     WHERE n.installed_in_node = $1 AND n.write_off_date IS NULL
     ORDER BY n.name`, [nodeId]);
    return result.rows;
}
export async function getNodeMovementHistory(nodeId) {
    const result = await query(`SELECT history_id, previous_location, new_location, moved_at,
     ...
     WHERE mh.node_id = $1
     ORDER BY mh.moved_at DESC`, [nodeId]);
    return result.rows;
}
export async function createNode(nodeData, userId, ipAddress, userAgent) {
    const client = await getClient();
    try {
        await client.query('BEGIN');
        // Проверяем, существует ли подсистема
        const subsystemCheck = await client.query(`SELECT subsys_id FROM equipment.subsystems WHERE subsys_id = $1`, [nodeData.subsystem_id]);
        ...
        if (nodeData.installed_in_node) {
            const parentCheck = await client.query(`SELECT node_id FROM equipment.nodes WHERE node_id = $1 AND write_off_date IS NULL`, [nodeData.installed_in_node]);
        ...
        const result = await client.query(`INSERT INTO equipment.nodes (
...
export async function updateNode(nodeId, nodeData, userId, ipAddress, userAgent) {
    const client = await getClient();
    try {
        await client.query('BEGIN');
        ...
        values.push(nodeId);
        await client.query(`UPDATE equipment.nodes SET ${updates.join(', ')} WHERE node_id = $${paramIndex}`, values);
        await client.query('COMMIT');
...
export async function writeOffNode(nodeId, userId, ipAddress, userAgent) {
    const client = await getClient();
    try {
        await client.query('BEGIN');
        const oldData = await getNodeById(nodeId);
        ...
        await client.query(`UPDATE equipment.nodes SET write_off_date = CURRENT_DATE, status = 'списан' WHERE node_id = $1`, [nodeId]);
        await client.query('COMMIT');
