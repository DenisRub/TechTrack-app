...
export async function getAllInstruments(filters?: { status?: string; search?: string; location?: string }) {
  let sql = `
...
export async function getInstrumentById(instrumentId: string) {
...
export async function getInstrumentByTabNumber(tabNumber: string) {
  const result = await query(
    `SELECT instrument_id, name, manufacturer, model, tab_number, status
     FROM equipment.measuring_instruments
     WHERE tab_number = $1 AND status != 'списано'`,
    [tabNumber]
  );
  return result.rows[0] || null;
}

export async function getInstrumentCalibrationHistory(instrumentId: string) {
  const result = await query(
    `SELECT history_id, calibration_date, next_calibration_date,
...
export async function getInstrumentsExpiringSoon(daysThreshold: number = 30) {
  const result = await query(
    `SELECT mi.instrument_id, mi.name, mi.tab_number, mi.next_calibration_date,
...
export async function updateInstrument(
  ...
  await query(
    `UPDATE equipment.measuring_instruments SET ${updates.join(', ')} WHERE instrument_id = $${paramIndex}`,
    values
  );
...
export async function writeOffInstrument(instrumentId: string, userId: string | null, ipAddress: string | null, userAgent: string | null) {
  const oldData = await getInstrumentById(instrumentId);
  if (!oldData) {
    throw new Error('Средство измерения не найдено');
  }
  ...
  await query(
    `UPDATE equipment.measuring_instruments SET status = 'списано', updated_at = CURRENT_TIMESTAMP WHERE instrument_id = $1`,
    [instrumentId]
  );
