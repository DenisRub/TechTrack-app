<template>
  <div class="card" v-if="plan">
    ...
    <div class="plan-info">
      <p>
        <strong>Период:</strong> {{ formatDate(plan.startDate) }} — {{ formatDate(plan.endDate) }}
      </p>
        ...
        <thead>
          <tr>
            ...
            <th @click="sortBy('nodeName')">
              ...
              <span class="sort-icon" v-if="sortField === 'nodeName'">{{
                sortOrder === 'asc' ? '↑' : '↓'
              }}</span>
            </th>
            <th @click="sortBy('parentName')">
              Местоположение
              <span class="sort-icon" v-if="sortField === 'parentName'">{{
                sortOrder === 'asc' ? '↑' : '↓'
              }}</span>
            </th>
            <th @click="sortBy('expiryDate')">
              Дата истечения срока ТО
              <span class="sort-icon" v-if="sortField === 'expiryDate'">{{
                sortOrder === 'asc' ? '↑' : '↓'
              }}</span>
            </th>
            <th @click="sortBy('recommendedDate')">
              Дата проведения ТО
              <span class="sort-icon" v-if="sortField === 'recommendedDate'">{{
                sortOrder === 'asc' ? '↑' : '↓'
              }}</span>
            </th>
            <th @click="sortBy('serviceType')">
              Тип обслуживания
              <span class="sort-icon" v-if="sortField === 'serviceType'">{{
                sortOrder === 'asc' ? '↑' : '↓'
              }}</span>
            </th>
...
const sortField = ref<
  'nodeName' | 'parentName' | 'expiryDate' | 'recommendedDate' | 'serviceType' | 'status'
>('recommendedDate')
